...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
  ...
  if (i + 1 <= imax) ++imax;
  if (est.te_nota() and ((imax == -1) or (est.consultar_nota() > vest[imax].consultar_nota())
        or (est.consultar_nota() == vest[imax].consultar_nota() and i + 1 < imax)))
    imax = i + 1;
...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est) {
  ...
  else if (est.te_nota() and ((imax == -1) or (est.consultar_nota() > vest[imax].consultar_nota())
        or (est.consultar_nota() == vest[imax].consultar_nota() and i < imax)))
    imax = i;
...
l’original menys l’estudiant amb DNI = dni */
    ...
    if (x < imax) --imax;
    else if (x == imax) recalcular_posicio_imax();
...
int main () {
    ...
    cjt.estudiant_nota_max().escriure();
    int esb;
    cin >> esb;
    cjt.esborrar_estudiant(esb);
